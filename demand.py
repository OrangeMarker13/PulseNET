...
def create_demo_records(city=None, coordinates=None, count=DEMO_RECORD_COUNT):
    ...
    if coordinates:
        ...
        latitude, longitude = 35.2271, -80.8431
    centers = [(latitude, longitude), (latitude + .035, longitude - .04), (latitude - .04, longitude + .035), (latitude + .055, longitude + .045)]
    weights = [0.42, 0.25, 0.20, 0.13]
    selected = rng.choice(len(centers), size=count, p=weights)
    records = []
...
def create_demand_grid(records, latitude, longitude, grid_size=DEMAND_GRID_SIZE, radius=.12):
    if records is None or records.empty:
        return pd.DataFrame()
    minimum_latitude, maximum_latitude = latitude - radius, latitude + radius
    minimum_longitude, maximum_longitude = longitude - radius, longitude + radius
    filtered = records[records["latitude"].between(minimum_latitude, maximum_latitude) & records["longitude"].between(minimum_longitude, maximum_longitude)].copy()
    if filtered.empty:
        return pd.DataFrame()
    filtered["latitude_grid"] = np.floor((filtered["latitude"] - minimum_latitude) / (2 * radius) * grid_size).clip(0, grid_size - 1).astype(int)
    filtered["longitude_grid"] = np.floor((filtered["longitude"] - minimum_longitude) / (2 * radius) * grid_size).clip(0, grid_size - 1).astype(int)
...
def build_demand_zones(records, coordinates, grid_size=DEMAND_GRID_SIZE):
    if records is None or records.empty or not coordinates:
        return pd.DataFrame()
    latitude, longitude = coordinates
    data = prepare_demand_data(records)
    grid = create_demand_grid(data, latitude, longitude, grid_size)
    if grid.empty:
        return pd.DataFrame()
    radius = .12
